...
def make_pred_multilabel(data_transforms, model, PATH_TO_IMAGES, epoch_loss, CHROMOSOME):
    ...
    for column in true_df:
        ...
        total_auc+= thisrow['auc']

    avg_auc = total_auc / 14
    currentDT = datetime.datetime.now()
    pred_df.to_csv(f"results/val/preds_{epoch_loss}_{currentDT}_{CHROMOSOME}_{avg_auc}.csv", index=False)
    auc_df.to_csv(f"results/val/aucs_{epoch_loss}_{currentDT}_{CHROMOSOME}_{avg_auc}.csv", index=False)
